...
describe("ReportRepository", () => {
	...
	describe("save", () => {
		it.concurrent(
			...
			() => {
				repository.save("task-123", ReportType.REQUIREMENTS, "Test content");

				const retrieved = repository.get("task-123", ReportType.REQUIREMENTS);
				expect(retrieved).toBeDefined();
				expect(retrieved?.taskId).toBe("task-123");
				expect(retrieved?.reportType).toBe(ReportType.REQUIREMENTS);
		...
		it.concurrent(
			...
			() => {
				...
				repository.save("task-123", ReportType.REQUIREMENTS, "Updated content");
		...
		it.concurrent("should allow same taskId with different reportTypes", () => {
			...
			repository.save("task-123", ReportType.PLAN, "Plan content");
		...
		it.concurrent("should handle empty content string", () => {
			repository.save("task-123", ReportType.REQUIREMENTS, "");
		...
		it.concurrent("should handle large content", () => {
			...
			repository.save("task-123", ReportType.REQUIREMENTS, largeContent);
		...
		it.concurrent("should handle special characters in content", () => {
			...
			repository.save("task-123", ReportType.REQUIREMENTS, specialContent);
...
- Item 3
`;
			repository.save("task-123", ReportType.DOCUMENTATION, markdownContent);

			const retrieved = repository.get("task-123", ReportType.DOCUMENTATION);
		...
		it.concurrent("should handle JSON-like content in string", () => {
			...
			repository.save("task-123", ReportType.REQUIREMENTS, jsonContent);
	...
	describe("get", () => {
		...
		it.concurrent(
			...
			() => {
				repository.save("task-1", ReportType.REQUIREMENTS, "Content 1");
				repository.save("task-1", ReportType.PLAN, "Content 2");
				repository.save("task-2", ReportType.REQUIREMENTS, "Content 3");

				expect(repository.get("task-1", ReportType.REQUIREMENTS)?.content).toBe(
					"Content 1"
				);
				expect(repository.get("task-1", ReportType.PLAN)?.content).toBe(
					"Content 2"
				);
				expect(repository.get("task-2", ReportType.REQUIREMENTS)?.content).toBe(
					"Content 3"
				);
			}
		);
	...
	describe("clear", () => {
		it.concurrent("should remove all reports from repository", () => {
			repository.save("task-1", ReportType.REQUIREMENTS, "Content 1");
			repository.save("task-2", ReportType.PLAN, "Content 2");
		...
		it.concurrent("should allow save after clear", () => {
			...
			repository.save("task-2", ReportType.PLAN, "New content");

			expect(repository.get("task-2", ReportType.PLAN)?.content).toBe(
				"New content"
			);
		});
	});
